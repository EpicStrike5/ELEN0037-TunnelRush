...
-- start_screen.vhd
--
-- Input   : Clock and per-frame tick
--           SNES button state (reserved, unused)
-- Output  : Position, size, activity and colour for 6 animated particles :
--
--					sq_px / sq_py — signed screen centre per particle
--					sq_half       — half-size (radius) per particle
--					sq_active     — live flag per particle
--					sq_col        — colour index per particle
--
-- Utility : Generates 6 coloured squares flying outward from screen centre.
--           Runs continuously; output consumed by screen_overlay.vhd
--           only during IDLE state
--
...
entity start_screen is
    port (
        ...
        sq_px     : out std_logic_vector(65 downto 0);
        sq_py     : out std_logic_vector(65 downto 0);
        sq_half   : out std_logic_vector(47 downto 0);
        sq_active : out std_logic_vector(5  downto 0);
        sq_col    : out std_logic_vector(17 downto 0)
    );
end start_screen;
...
architecture rtl of start_screen is
    ...
    constant GROW_STEP  : integer := 2;    -- half-size growth per frame
    constant LERP_SHIFT : integer := 4;    -- position moves 1/16 of remaining gap per frame
    ...
    signal s_tx     : pos_arr  := (others => to_signed(CX, 11));
    signal s_ty     : pos_arr  := (others => to_signed(CY, 11));
    signal s_half   : half_arr := (others => (others => '0'));
    signal s_active : bool_arr := (others => '0');
    signal s_col    : col_arr  := (others => (others => '0'));

    -- 16-bit Galois LFSR for pseudo-random target generation
...
begin
    ...
    -- Flatten particle arrays onto output buses (particle i at width*(i+1)-1 downto width*i)
    ...
    begin
        if rising_edge(CLK_50) then
            ...
            -- LFSR advances every clock for best entropy
            if lfsr(0) = '1' then
            ...
            if frame_tick = '1' then
                ...
                for i in 0 to NUM_SQ-1 loop
                    if s_active(i) = '1' then
                        ...
                        -- Grow half-size; deactivate when maximum is reached
                        ...
                        -- Integer lerp: move 1/16 of remaining distance toward target
                        dx     := s_tx(i) - s_px(i);
                        dy     := s_ty(i) - s_py(i);
                        step_x := shift_right(dx, LERP_SHIFT);
                        step_y := shift_right(dy, LERP_SHIFT);

                        -- Guarantee at least 1 px movement when not yet at target
                ...
                -- Every SPAWN_GAP frames: fill the first available inactive slot
