...
-- RushTunnelFPGA.vhd
--
-- Input   : CLK_50 — 50 MHz board clock
--           GP_D_1 — SNES controller serial data
--
-- Output  : RED, GREEN, BLUE (4b each) — VGA colour channels
--           SYNC (2b) — SYNC(0)=vsync  SYNC(1)=hsync
--           GP_L_1, GP_C_1 — SNES latch and clock
--
-- Utility : Top-level entity.  Wires all sub-modules together and
--           generates the :
--
--					frame_tick
--					scroll
--					hue_cnt signals.
--
--           Module chain: vga_controller → tunnel → crt_effects
--           → screen_overlay → VGA pins
-- 
...
architecture rtl of RushTunnelFPGA is
    ...
    constant SPEED_DIV    : integer := 1;       -- scroll increments every N frames
    constant SHIP_RADIUS  : integer := 285;     -- ship orbit radius (px from vanishing point)
    ...
    signal wave_tick_s                  : std_logic;

    -- Pixel-based collision from tunnel renderer
    signal ship_in_obs_s     : std_logic := '0';
    signal ship_on_coin_s    : std_logic := '0';
    signal ship_on_coin_prev : std_logic := '0';
    ...
    -- Fixed octagon face count for obstacle_manager
    constant NUM_FACES_OCT : std_logic_vector(3 downto 0) := "1000";
...
begin
    ...
    -- Trig ROM instance for tunnel rotation angle
    ...
    -- Trig ROM instance for ship screen angle
    ...
    -- Ship screen position: (400 + cos_s×R/256, 300 + sin_s×R/256)
    process(CLK_50)
    ...
    -- Octagon tunnel renderer (2-clock pipeline)
    ...
    -- Diagonal criss-cross grid post-processing (1-clock pipeline)
    ...
    -- Start-screen particle animation (runs continuously, used only in IDLE)
    ...
    -- Final UI overlay: start screen / tunnel pass-through / game-over (1-clock pipeline)
    ...
    -- Game FSM: angular physics, inertia, difficulty ramp
    u_ship : entity work.spaceship
        port map (
            clk_50         => CLK_50,
            frame_tick     => frame_tick,
            buttons        => buttons,
            collision      => ship_in_obs_s,
            wave_tick      => wave_tick_s,
            game_state     => game_state,
    ...
    -- Obstacle and coin wave manager
    ...
    -- Frame counter: generates frame_tick, scroll, and hue_cnt
    process(CLK_50)
    begin
        if rising_edge(CLK_50) then
            frame_tick <= '0';
    ...
    -- BCD score counter: increments on rising edge of ship_on_coin_s; resets on new game
    process(CLK_50)
    begin
        if rising_edge(CLK_50) then
